...
| 32, 33 | Persistence & snapshots | `services/trie/snapshot`, `services/trie/worker.py` (replay on start), `services/trie/service.py`, `packages/hammertime-bus` (`ConsumedMessage.offset`, positional `subscribe`, `MessageBus.end_offset` and `first_offset`), `tools/provision`, `docs/adr/0010` (Amendments 1, 2), `docs/adr/0013` (decisions 2, 9; Amendment 10), `docs/adr/0017` (decisions 3, 4, 8 and 16; Amendment 2: the replay re-publishes the stats it replays) |
