...
| 13, 38 | `HOT_PREFIX` predicate (single implementation) | `core/state/prefix.py`, `services/detector/rules/baseline.py`, `services/trie/query`, `docs/adr/0010`, `docs/adr/0017` (Amendment 4 ruling 1: the predicate's arguments) |
| 13, 14, 31 | Prefix classification & scoring | `services/detector`, `services/trie/query` (`GET /prefixes/hot`, `?minimal=true` for §31), `docs/adr/0017` (Amendment 4 rulings 2 and 5; Amendment 5 ruling 2: the minimal set's granularity, the owner's decision) |
| 16, 17 | Prefix metadata inheritance | `services/trie/metadata` (`combine.py`, `local.py`), `docs/adr/0015` |
| 19 | Event-driven internals | `core/events`, `packages/hammertime-bus` (`interface.py`, `memory.py`, `nats.py`), `tools/provision`, `services/trie/worker.py` (the hot-ip consumer), `services/trie/publisher.py` (the prefix-stats producer), `docs/adr/0004`, `docs/adr/0013`, `docs/adr/0015` (Amendment 3 ruling 3: the codec's integer fields; Amendment 5: `window_count` is required on hot-ip events), `docs/adr/0016` (decisions 1 and 2: the schemas' numeric bounds; Amendment 1: `hot_ratio`), `docs/adr/0017` (decisions 3, 6 and 14; Amendment 2: the publisher; Amendment 3: a start re-publishes only from the prefix-stats log's last sequence, and the codec returns a decoded timestamp in UTC) |
...
| 29 | Read path | `services/trie/query`, `services/detector/api.py`, `docs/protocol/read-api-v1.md`, `docs/adr/0010`, `docs/adr/0015` (Amendment 5: `request_count` on `GET /ip`), `docs/adr/0017` (decisions 9 and 15; Amendment 4: the read API as designed) |
...
| 32, 33 | Persistence & snapshots | `services/trie/snapshot`, `services/trie/worker.py` (replay on start), `services/trie/service.py`, `packages/hammertime-bus` (`ConsumedMessage.offset`, positional `subscribe`, `MessageBus.end_offset`, `first_offset` and `last_value`), `tools/provision`, `docs/adr/0010` (Amendments 1, 2, 4), `docs/adr/0013` (decisions 2, 9; Amendments 10 and 13), `docs/adr/0017` (decisions 3, 4, 8 and 16; Amendment 2: the replay re-publishes the stats it replays; Amendment 3: only from the last sequence the prefix-stats log holds) |
| 34 | Versioned configuration | `core/config`, `services/aggregator/reevaluate.py`, `docs/adr/0011` |
| 35 | IPv6 readiness | `core/addressing`, `services/trie/config.py` (`HAMMERTIME_TRIE_FAMILIES`; `HAMMERTIME_TRIE_MIN_PREFIX_LENGTH` and `HAMMERTIME_TRIE_MIN_PREFIX_LENGTH_IPV6`, the reporting floor per family), `services/trie/state.py`, `services/trie/publisher.py`, `docs/adr/0010` (Amendment 3), `docs/adr/0017` (decision 5; Amendment 2 ruling 1: IPv6 reports `/104` to `/128`) |
...
| 37 | Observability | `core/telemetry`, `services/aggregator/metrics.py`, `services/trie/metrics.py`, `deploy/grafana`, `docs/adr/0017` (decision 12; Amendment 2 ruling 7: `prefix_stats_published`, and why slice 2 does not measure `hot_transition_to_prefix_update_latency`; Amendment 3 ruling 1: `replay_complete`'s `republish_from` and the `prefix_stats_last_ignored` record; Amendment 4 ruling 9: `prefix_queries`) |
| 43 | Recommended initial implementation; reference components (event log, store, images) and their licence policy | `deploy/docker-compose.yml`, `docs/adr/0012` (Amendment 2), `docs/adr/0013` (decisions 11, 12; Amendment 14: the services' own ports), `docs/adr/0017` (Amendment 5 ruling 1: `8081` to `8083` and `9090` on `127.0.0.1`) |
